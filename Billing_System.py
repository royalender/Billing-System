...
def save_to_db(bill_number, dosa, coffee, tea, biryani, chapati, puri, total_price):
    connection = connect_db()
    cursor = connection.cursor()
    sql = "INSERT INTO bills (bill_number, dosa, coffee, tea, biryani, chapati, puri, total_price) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
    values = (bill_number, dosa, coffee, tea, biryani, chapati, puri, total_price)
    
    try:
        cursor.execute(sql, values)
        connection.commit()
        messagebox.showinfo("info", "BILL SAVED TO DATABASE SUCCESSFULLY")
...
def save():
    if o==35:
        ...
        save_to_db(a, a1, a2, a3, a4, a5, a6, o)
...
def search_bill():
    ...
    def sea():
        bill_number = entry_search.get()
        connection = connect_db()
        cursor = connection.cursor()

        sql = "SELECT * FROM bills WHERE bill_number = %s"
        cursor.execute(sql, (bill_number,))
        result = cursor.fetchone()

        Z.delete(1.0, END)  # Clear previous bill

        if result:
            # Display the bill details
            Z.insert(END, '************************ BILL DETAILS **********************\n')
            Z.insert(END, f'Bill Number: {result[1]}\n')
            Z.insert(END, f'Dosa: {result[2]}\n')
            Z.insert(END, f'Coffee: {result[3]}\n')
            Z.insert(END, f'Tea: {result[4]}\n')
            Z.insert(END, f'Biryani: {result[5]}\n')
            Z.insert(END, f'Chapati: {result[6]}\n')
            Z.insert(END, f'Puri: {result[7]}\n')
            Z.insert(END, f'Total Price: {result[8]} Rs\n')
            Z.insert(END, '***************************************************************\n')
        else:
